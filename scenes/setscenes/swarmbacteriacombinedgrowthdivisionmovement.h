#include <random>
class SwarmBacterCombinedGrowthDivisionMovement : public Scene
{

public:
	SwarmBacterCombinedGrowthDivisionMovement(const char* name) : Scene(name) {}
	...
	virtual void Initialize()
	{
		...
		for (int i = 0; i < cellCount; ++i)
		{
			...
			bac.grow = 0;
			bac.divide = 0;
			bac.cell_velocity = mVelocity;
			bac.growth_probability = growthProb;
	...
	virtual void DoGui() // Include a parameter varying GUI
	{
		...
		if (imguiSlider("Bacteria Growth Probability", &growthProb, 0.0f, 0.02f, 0.001f))
		{
			for (int i = 0; i < int(g_bac.size()); ++i)
				g_bac[i].growth_probability = growthProb;
		}
	}
	...
	void Update()
	{
		...
		for (int cell_counter = 0; cell_counter < cellCount; cell_counter++) {
			...
			if (grow_prob < g_bac[cell_counter].growth_probability & g_bac[cell_counter].number_particles <= maxCellParticles) {
